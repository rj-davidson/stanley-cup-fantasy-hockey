...
func (sq *StatsQuery) Order(o ...stats.OrderOption) *StatsQuery {
	sq.order = append(sq.order, o...)
	return sq
}

// QueryPlayer chains the current query on the "player" edge.
func (sq *StatsQuery) QueryPlayer() *PlayerQuery {
...
// Returns a *NotSingularError when more than one Stats entity is found.
...
// Returns a *NotSingularError when more than one Stats ID is found.
...
func (sq *StatsQuery) All(ctx context.Context) ([]*Stats, error) {
	...
	qr := querierAll[[]*Stats, *StatsQuery]()
	return withInterceptors[[]*Stats](ctx, sq, qr, sq.inters)
}
...
func (sq *StatsQuery) IDs(ctx context.Context) (ids []int, err error) {
...
func (sq *StatsQuery) Count(ctx context.Context) (int, error) {
	...
	return withInterceptors[int](ctx, sq, querierCount[*StatsQuery](), sq.inters)
}
...
func (sq *StatsQuery) Clone() *StatsQuery {
	...
	return &StatsQuery{
		config:     sq.config,
		ctx:        sq.ctx.Clone(),
		order:      append([]stats.OrderOption{}, sq.order...),
...
func (sq *StatsQuery) WithPlayer(opts ...func(*PlayerQuery)) *StatsQuery {
	query := (&PlayerClient{config: sq.config}).Query()
...
//
//	client.Stats.Query().
//		Select(stats.FieldGoals).
//		Scan(ctx, &v)
func (sq *StatsQuery) Select(fields ...string) *StatsSelect {
	sq.ctx.Fields = append(sq.ctx.Fields, fields...)
	sbuild := &StatsSelect{StatsQuery: sq}
	sbuild.label = stats.Label
	sbuild.flds, sbuild.scan = &sq.ctx.Fields, sbuild.Scan
	return sbuild
}

// Aggregate returns a StatsSelect configured with the given aggregations.
func (sq *StatsQuery) Aggregate(fns ...AggregateFunc) *StatsSelect {
...
func (sq *StatsQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Stats, error) {
	var (
		nodes       = []*Stats{}
		_spec       = sq.querySpec()
		loadedTypes = [1]bool{
	...
	_spec.ScanValues = func(columns []string) ([]any, error) {
		return (*Stats).scanValues(nil, columns)
	}
	...
	if query := sq.withPlayer; query != nil {
		if err := sq.loadPlayer(ctx, query, nodes, nil,
			func(n *Stats, e *Player) { n.Edges.Player = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (sq *StatsQuery) loadPlayer(ctx context.Context, query *PlayerQuery, nodes []*Stats, init func(*Stats), assign func(*Stats, *Player)) error {
	...
	query.Where(predicate.Player(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(stats.PlayerColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.stats_player
		if fk == nil {
			return fmt.Errorf(`foreign-key "stats_player" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "stats_player" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (sq *StatsQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(stats.Table, stats.Columns, sqlgraph.NewFieldSpec(stats.FieldID, field.TypeInt))
	_spec.From = sq.sql
...
func (sgb *StatsGroupBy) Scan(ctx context.Context, v any) error {
	ctx = setContextOp(ctx, sgb.build.ctx, "GroupBy")
	if err := sgb.build.prepareQuery(ctx); err != nil {
		return err
	}
	return scanWithInterceptors[*StatsQuery, *StatsGroupBy](ctx, sgb.build, sgb, sgb.build.inters, v)
}

func (sgb *StatsGroupBy) sqlScan(ctx context.Context, root *StatsQuery, v any) error {
...
func (ss *StatsSelect) Aggregate(fns ...AggregateFunc) *StatsSelect {
	ss.fns = append(ss.fns, fns...)
	return ss
}

// Scan applies the selector query and scans the result into the given value.
func (ss *StatsSelect) Scan(ctx context.Context, v any) error {
	ctx = setContextOp(ctx, ss.ctx, "Select")
	if err := ss.prepareQuery(ctx); err != nil {
		return err
	}
	return scanWithInterceptors[*StatsQuery, *StatsSelect](ctx, ss.StatsQuery, ss, ss.inters, v)
}

func (ss *StatsSelect) sqlScan(ctx context.Context, root *StatsQuery, v any) error {
