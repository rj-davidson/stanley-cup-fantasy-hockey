...
func (tc *TeamCreate) SetLogoFilepath(s string) *TeamCreate {
	tc.mutation.SetLogoFilepath(s)
	return tc
}

// SetEliminated sets the "eliminated" field.
func (tc *TeamCreate) SetEliminated(b bool) *TeamCreate {
	tc.mutation.SetEliminated(b)
	return tc
}

// SetNillableEliminated sets the "eliminated" field if the given value is not nil.
func (tc *TeamCreate) SetNillableEliminated(b *bool) *TeamCreate {
	if b != nil {
		tc.SetEliminated(*b)
	}
	return tc
...
func (tc *TeamCreate) defaults() {
	if _, ok := tc.mutation.Eliminated(); !ok {
		v := team.DefaultEliminated
		tc.mutation.SetEliminated(v)
	}
}
...
func (tc *TeamCreate) createSpec() (*Team, *sqlgraph.CreateSpec) {
	...
	if value, ok := tc.mutation.LogoFilepath(); ok {
		_spec.SetField(team.FieldLogoFilepath, field.TypeString, value)
		_node.LogoFilepath = value
	}
	if value, ok := tc.mutation.Eliminated(); ok {
		_spec.SetField(team.FieldEliminated, field.TypeBool, value)
		_node.Eliminated = value
	}
