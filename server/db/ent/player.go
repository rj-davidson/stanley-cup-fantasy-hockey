...
// was not loaded in eager-loading.
func (e PlayerEdges) GameStatsOrErr() ([]*GameStats, error) {
	if e.loadedTypes[3] {
...
func (*Player) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case player.ForeignKeys[0]: // stats_player
			values[i] = new(sql.NullInt64)
		case player.ForeignKeys[1]: // team_players
			values[i] = new(sql.NullInt64)
		default:
	...
	for i := range columns {
		...
		case player.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for edge-field stats_player", value)
			} else if value.Valid {
				pl.stats_player = new(int)
				*pl.stats_player = int(value.Int64)
			}
...
// QueryGameStats queries the "gameStats" edge of the Player entity.
func (pl *Player) QueryGameStats() *GameStatsQuery {
	return NewPlayerClient(pl.config).QueryGameStats(pl)
}
