...
func (eq *EntryQuery) QueryLeague() *LeagueQuery {
	query := (&LeagueClient{config: eq.config}).Query()
...
func (eq *EntryQuery) WithLeague(opts ...func(*LeagueQuery)) *EntryQuery {
	query := (&LeagueClient{config: eq.config}).Query()
...
func (eq *EntryQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Entry, error) {
	...
	if query := eq.withLeague; query != nil {
		if err := eq.loadLeague(ctx, query, nodes, nil,
			func(n *Entry, e *League) { n.Edges.League = e }); err != nil {
			return nil, err
		}
	}
...
func (eq *EntryQuery) loadLeague(ctx context.Context, query *LeagueQuery, nodes []*Entry, init func(*Entry), assign func(*Entry, *League)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Entry)
	for i := range nodes {
		if nodes[i].league_entries == nil {
			continue
		}
		fk := *nodes[i].league_entries
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "league_entries" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
